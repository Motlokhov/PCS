...
namespace PCS_Forms.Core
{
    ...
    public class Parameter
    {
        ...
        public static Parameter Construct(int id) 
        {
        ...
        private void AddValues()
        {
            ...
            for (int i = 0; i < Values.Length; i++)
            {
                ...
                Values[i] = Value.Construct(id,this.Limitation, this.Type,isSum);
            }
        }
        ...
        public void Interpretate()
        {
            switch (Rule)
            {
                case RuleInterpretationParameter.none:
                    Interpretation = InterpretateAsNone();
                    break;
                case RuleInterpretationParameter.single:
                    Interpretation = InterpretateAsSingle();
                    break;
                case RuleInterpretationParameter.group:
                    Interpretation = InterpretateAsGroup();
                    break;
            }
        }
        ...
        public void CreateStackPanel(MyGroupBox groupbox)
        {
            MyStackPanel stackPanel = new MyStackPanel(System.Windows.Controls.Orientation.Horizontal);
            stackPanel.ToolTip = this.Name + " (Лимит значения: " + this.Limitation + ")";
            foreach (Value value in Values)
            {
                value.CreateTextBox(stackPanel);
            }
            groupbox.AddElement(stackPanel);
